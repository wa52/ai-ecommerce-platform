...
| 商品发现 | 分类、品牌、标签 | 分类导航、品牌筛选、结果数量 | 未实现 |
| 商品发现 | 搜索、联想、排序、筛选 | 关键词、价格、库存、相关性排序 | 部分实现：已有基础搜索 |
| 商品发现 | 推荐、热销、新品 | 基于商品和行为数据展示推荐位 | 未实现 |
| 商品详情 | 图库、封面、详情富文本 | 多图、缩略图、商品描述和参数 | 部分实现：已有单图和基础描述 |
| 商品详情 | Variant / SKU 属性 | 规格选择、库存、价格联动 | 部分实现 |
| 商品详情 | 收藏、浏览记录、猜你喜欢 | 登录后持久化，匿名可记录本地历史 | 未实现 |
| 交易 | 购物车、地址、配送、订单确认 | Saleor Checkout 真实链路 | 已实现 |
| 交易 | 支付 | 独立支付页、支付状态和幂等处理 | 部分实现：当前为 Dummy 沙箱 |
| 交易 | 优惠券、满减、促销、运费规则 | 结算前可试算，订单保存优惠快照 | 未实现 |
| 交易 | 发票 | 发票抬头、税号、订单关联 | 未实现 |
| 用户中心 | 注册、登录、账户首页 | 消费者账户与订单关联 | 部分实现 |
| 用户中心 | 地址簿 | 新增、编辑、删除、默认地址 | 未实现 |
| 用户中心 | 账户安全 | 修改密码、退出登录、异常提示 | 部分实现 |
| 订单 | 订单列表和详情 | 按状态查看、金额、商品、支付状态 | 部分实现 |
...
| 订单 | 订单列表、详情、状态操作 | 搜索、筛选、备注、取消、发货 | 未实现消费者之外的完整后台流程 |
| 库存 | 库存、库存流水、预警 | 可用库存、锁定库存、调整记录 | 部分实现：后端模型/接口优先 |
...
| AI Center | Agent、知识库、客服 | 工具权限、引用来源、失败可追踪 | 部分实现：需接入 Storefront |
