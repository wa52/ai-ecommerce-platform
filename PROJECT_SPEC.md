...
商城不是只有“商品列表到下单”的单一链路。后续实现、验收和后台菜单必须以以下四个业务域为准；每个模块都要区分“已实现、部分实现、未实现”，不能因为底层接口存在就把消费者功能标记为完成。

状态定义：

``` text
已实现       页面、接口、数据和主要异常流程已连通
部分实现     有基础链路，但缺少完整业务能力或正式第三方验证
未实现       尚无可用的正式业务流程
受外部条件阻塞 代码已具备，但需要商户资质、第三方账号或生产配置
```
...
| 业务域 | 模块 | V1 要求 | 当前状态 |
| --- | --- | --- | --- |
| 商品发现 | 首页、商品列表、中文商品数据 | Saleor 商品真实读取，分页和空态 | 已实现 |
| 商品发现 | 分类、品牌、标签 | 分类导航、品牌筛选、结果数量 | 部分实现：已有 Saleor 分类筛选 |
| 商品发现 | 搜索、联想、排序、筛选 | 关键词、价格、库存、相关性排序 | 部分实现：已有关键词、价格排序和有货筛选 |
| 商品发现 | 推荐、热销、新品 | 基于商品和行为数据展示推荐位 | 未实现 |
| 商品详情 | 图库、封面、详情富文本 | 多图、缩略图、商品描述和参数 | 部分实现：图库降级和基础描述已接入 |
| 商品详情 | Variant / SKU 属性 | 规格选择、库存、价格联动 | 部分实现：规格选择和库存提示已接入 |
| 商品详情 | 收藏、浏览记录、猜你喜欢 | 登录后持久化，匿名可记录本地历史 | 部分实现：收藏和收藏页已接入 |
| 交易 | 购物车、地址、配送、订单确认 | Saleor Checkout 真实链路 | 已实现 |
| 交易 | 支付 | 独立支付页、支付状态和幂等处理 | 部分实现：当前为 Dummy 沙箱 |
| 交易 | 优惠券、满减、促销、运费规则 | 结算前可试算，订单保存优惠快照 | 部分实现：优惠码已接 Saleor |
| 交易 | 发票 | 发票抬头、税号、订单关联 | 未实现 |
| 用户中心 | 注册、登录、账户首页 | 消费者账户与订单关联 | 部分实现 |
| 用户中心 | 地址簿 | 新增、编辑、删除、默认地址 | 部分实现：新增/删除已接 Saleor |
| 用户中心 | 账户安全 | 修改密码、退出登录、异常提示 | 部分实现 |
| 订单 | 订单列表和详情 | 按状态查看、金额、商品、支付状态 | 部分实现：账户页已有状态筛选 |
| 订单 | 取消、再次购买、确认收货 | 状态约束和幂等操作 | 未实现 |
| 售后 | 退款、退货、换货、售后进度 | 申请、审核、退款结果可追踪 | 部分实现：后台退款申请、退款记录和金额上限已接入 |
| 履约 | 物流信息、运单追踪 | 发货、物流公司、轨迹、确认收货 | 部分实现：Saleor 发货单和运单号展示已接入，第三方轨迹未接 |
| 服务 | 配送、售后、支付安全、客服 | 帮助中心和常见问题 | 已实现基础版 |
| AI 体验 | AI 搜索、导购、问答、客服、售后助手 | 消费者能在商城中直接使用 AI | 未实现 |

### B. 商家后台

| 业务域 | 模块 | V1 要求 | 当前状态 |
| --- | --- | --- | --- |
| 商品 | 商品、分类、品牌、SKU、图片 | CRUD、批量操作、上架/下架 | 部分实现：已有商品管理基础链路 |
| 订单 | 订单列表、详情、状态操作 | 搜索、筛选、备注、取消、发货 | 部分实现：后台订单列表已接真实 API |
| 库存 | 库存、库存流水、预警 | 可用库存、锁定库存、调整记录 | 部分实现：后台库存列表已接真实 API |
| 客户 | 客户列表、客户详情、分群 | 订单、消费、标签和权限隔离 | 部分实现：Saleor 客户列表已接入 |
| 营销 | 优惠券、促销、满减、活动 | 规则配置、使用限制、效果统计 | 未实现 |
| 售后 | 售后单、退款审核、原因统计 | 审核、退款、状态和审计记录 | 部分实现：后台退款审核入口、金额校验、财务账本已接入 |
| 物流 | 配送模板、发货、物流商 | 运单号、发货通知、轨迹同步 | 部分实现：Saleor 发货操作和运单号展示已接入 |
| 支付 | 支付配置、交易、退款 | 支付渠道、回调、异常和幂等 | 部分实现：协议层和沙箱已具备 |
| 财务 | 收款、退款、结算、对账、利润 | Decimal 金额、账本和审计 | 部分实现 |
| 内容 | Banner、专题页、装修、导航 | 内容发布和预览 | 未实现 |
| 分析 | 销售、商品、客户、利润、漏斗 | 统一指标口径和时间筛选 | 部分实现 |
| AI Center | 文案、翻译、分析、推荐 | 从真实商品/订单数据生成结果 | 部分实现：基础设施已有 |
| AI Center | Agent、知识库、客服 | 工具权限、引用来源、失败可追踪 | 部分实现：后台 Agent 运行入口已接入 |
| 店铺设置 | 域名、支付、物流、通知、页面配置 | 独立部署和环境配置 | 部分实现 |
| 权限 | 用户、Role、Permission、审计 | 后端 RBAC，不只隐藏前端按钮 | 部分实现 |

### C. AI 能力域

AI 不是独立展示页，而是建立在真实业务数据之上的能力层：

``` text
消费者行为 / 商品 / 订单 / 库存 / 售后 / 知识库
                         ↓
              FastAPI AI Extension Layer
                         ↓
       AI 搜索 · AI 导购 · 推荐 · 客服 · 售后助手
                         ↓
             Agent Tools / RAG / LLM Gateway
```

所有 AI 结果必须能说明数据来源；没有业务数据时不得编造销售额、库存、利润、物流或售后状态。AI 层不得复制 Saleor 的商品、购物车、订单和支付核心模型。

### D. 平台基础能力

| 能力 | 要求 | 当前状态 |
| --- | --- | --- |
| 身份与权限 | IAM、RBAC、店铺数据隔离、敏感凭据保护 | 部分实现 |
| 数据与任务 | PostgreSQL、Redis、Worker、Migration、审计 | 部分实现 |
| 支付与财务 | Payment、Refund、Settlement、Reconciliation | 部分实现，真实支付宝待配置 |
| 第三方连接 | Shopify/Amazon 等 Connector、同步幂等和重试 | 部分实现：Fake/Sandbox 优先 |
| 可观测性 | 日志、Request ID、任务状态、错误追踪 | 部分实现 |
| 安全与交付 | Secret Scan、CORS、Webhook 验签、部署配置 | 部分实现 |
...
当前已完成的基线是：平台基础骨架、Saleor Commerce Core、FastAPI AI 扩展层、基础 Admin、消费者 Storefront 主购买链路，以及支付页/服务保障基础页。当前基线仍属于“可购买的商城 Demo”，不等于完整商城完成。

后续不按孤立页面推进，而按能够形成业务闭环的业务域推进。每个阶段都必须同步更新功能矩阵、测试和验收报告。
...
### Phase 11 --- 消费者 Storefront 主链路（已完成基础版）
...
验收重点：真实 Saleor 数据、分页、空态、错误态、图片降级、移动端布局，以及筛选条件刷新后仍然可复现。
...
验收重点：优惠计算与订单快照、重复提交、支付回调幂等、地址权限、支付失败恢复。支付宝在商户配置完成前只能标记为 `BLOCKED` 或 `Sandbox`，不得写成生产支付通过。
...
验收重点：状态转换合法性、库存回滚、退款金额不超额、重复 webhook、物流同步失败重试。
...
验收重点：商家只能访问授权店铺；后台操作必须真实写入数据库；危险操作需要确认并记录审计日志。
...
验收重点：Agent 必须调用真实 Tool；Tool 失败时明确提示；答案带来源；无数据不得编造；消费者和商家权限分别生效。
...
验收重点：统一指标公式、已知输入数据对账、完整回归、生产配置检查和可复查证据。
